...
die("Usage:[-a $opts{a}] [-b $opts{b}] [-c $opts{c}] [-d $opts{d}] [-e $opts{e}] [-f $opts{f}] [-g $opts{g}]  [-h $opts{h}] [-r $opts{r}] [-t $opts{t}] [-k $opts{u}] \n") if (@ARGV eq '' && -t STDIN);

my $install_directory = ".";
my $bedtools_dir = "/usr/local3/bin/bedtools-iplant-1.00/";
